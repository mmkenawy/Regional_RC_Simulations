...
############################################################################################
#
proc rotSpring2DModIKModel {eleID nodeR nodeC K asPos asNeg MyPos MyNeg LS LC LA LK cS cC cA cK th_pP th_pN th_pcP th_pcN ResP ResN th_uP th_uN DP DN} {
#
# Create the zero length element
      #uniaxialMaterial Bilin  $eleID  $K  $asPos $asNeg $MyPos $MyNeg $LS $LC $LA $LK $cS $cK $cA $cD $th_pP $th_pN $th_pcP $th_pcN $ResP $ResN $th_uP $th_uN $DP $DN;
	  uniaxialMaterial ModIMKPeakOriented $eleID $K $asPos $asNeg $MyPos $MyNeg $LS $LC $LA $LK $cS $cC $cA $cK $th_pP $th_pN $th_pcP $th_pcN $ResP $ResN $th_uP $th_uN $DP $DN;
	  
	  #puts "uniaxialMaterial ModIMKPeakOriented $eleID $K $asPos $asNeg $MyPos $MyNeg $LS $LC $LA $LK $cS $cC $cA $cK $th_pP $th_pN $th_pcP $th_pcN $ResP $ResN $th_uP $th_uN $DP $DN;"
